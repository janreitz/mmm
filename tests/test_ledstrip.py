...
assert not any(is_breathing_frame(f) for f in during_animation), \
    "breathing interrupted the volume animation"
assert any(len(set(f)) > 2 for f in during_animation), \
    "no volume bar frames seen"

after_animation = frames_between(t1 + 2.0, time.monotonic())
assert any(is_breathing_frame(f) for f in after_animation), \
    "breathing did not resume after the volume animation"
...
assert not any(is_breathing_frame(f) for f in late), \
    "breathing kept running after clear()"
